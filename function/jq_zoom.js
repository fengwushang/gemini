...
$.fn.extend({
    btbAd: function () {
        ...
        $(settings.menu + " li").eq(0).css({ "background": "#ff0000", "color": "#fff", "border": "1px solid #eaeaea" });
        $(settings.menu + " li").mouseover(function () {
            var i = $(this).text() - 1;
            n = i;
            if (n >= count) return;
            $("a", obj).filter(":visible").fadeOut(50, function () { $(this).parent().children().eq(n).fadeIn(100); });
            $(this).css({ "background": "#ff0000", "color": "#fff", "border": "1px solid #eaeaea" }).siblings().css({ "background": "#416fa0", "color": "#fff", "border": "1px solid #eaeaea" });
        });
